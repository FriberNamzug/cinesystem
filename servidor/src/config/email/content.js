import {CLIENTE, APP_NAME} from '../config.js'
export const activarCuenta = (nombre, token) => `
    <div style="background-color: #f5f5f5; padding: 20px; border-radius: 10px; text-align: center; font-family: sans-serif;">
        ...
        <a href="${CLIENTE}/activar-cuenta/${token}/" style="background-color: #e84393; color: #fff; padding: 10px 20px; border-radius: 5px; text-decoration: none;">Activar cuenta</a>
        <p>Si no puedes dar click en el enlace copia y pega la siguiente dirección en tu navegador</p>
        <p style="color: #2d3436;">${CLIENTE}/activar-cuenta/${token}/</p>
        </div>
    `
...
export const recuperarPasswordEmail = (nombre, token) => `
    <div style="background-color: #f5f5f5; padding: 20px; border-radius: 10px; text-align: center; font-family: sans-serif;">
        ...
        <a href="${CLIENTE}/recuperar_password/${token}/" style="background-color: #e84393; color: #fff; padding: 10px 20px; border-radius: 5px; text-decoration: none;">Recuperar contraseña</a>
        <p>Si no puedes dar click en el enlace copia y pega la siguiente dirección en tu navegador</p>
        <p style="color: #2d3436;">${CLIENTE}/recuperar_password/${token}/</p>
        </div>
        <div>
...
export const ordenDeCompra = (nombre, folio, fecha, hora, descripcion, costo) => `
    <div style="background-color: #f5f5f5; padding: 20px; border-radius: 10px; text-align: center; font-family: sans-serif;">
        <h1 style="color: #2d3436;">Hola ${nombre} has realizado una compra en ${APP_NAME}</h1>
