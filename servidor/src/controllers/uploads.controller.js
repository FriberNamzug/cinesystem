...
export const uploadImage = async (req, res) => {
    try {
        ...
        if (type === "p") {
            const resultado = await pool.query(`INSERT INTO ${type === "p" ? "peliculas_imagenes" : "actores_imagenes"} (${type === "p" ? "id_pelicula" : "id_actor"}, url) VALUES (?, ?)`, [id, url], (error) => {
        ...
        if (type === "a") {
            const resultado = await pool.query(`UPDATE actores SET foto = ? WHERE id_actor = ?`, [url, id], (error) => {
                if (error) return res.status(500).json({ message: 'Error en la base de datos' })
            })
            if (resultado[0].affectedRows === 0) return res.status(400).json({ message: 'No se ha podido actualizar el registro en la base de datos' })
        }
...
export const getImages = async (req, res) => {
    try {
        ...
        const resultado = await pool.query(`SELECT url FROM ${type === "p" ? "peliculas_imagenes" : "actores_imagenes"} WHERE ${type === "p" ? "id_pelicula" : "id_actor"} = ?`, [id], (error) => {
            if (error) return res.status(500).json({ message: 'Error en la base de datos' })
        })
